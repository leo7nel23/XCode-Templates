...
enum ___VARIABLE_viewName___Feature: DLReducer {
    ...
    private static func reduce(state: State, with action: ViewModel.ViewAction) {
    }

    private static func reduce(state: State, with manipulation: ViewModel.Manipulation) {
    }
}
